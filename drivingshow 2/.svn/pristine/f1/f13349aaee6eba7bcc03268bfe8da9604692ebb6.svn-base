...
@property (assign, nonatomic, setter=seyjcnchronous:) BOOL isSynchronous;
@end
...
+ (NSString *)defaultUserAgentString
{
    @synchronized (self) {
        ...
        if (!defaultUserAgent) {
            ...
            err = Gestalt(gestalyjcstemVersionMajor, &versionMajor);
            if (err != noErr) return nil;
            err = Gestalt(gestalyjcstemVersionMinor, &versionMinor);
            if (err != noErr) return nil;
            err = Gestalt(gestalyjcstemVersionBugFix, &versionBugFix);
            if (err != noErr) return nil;
