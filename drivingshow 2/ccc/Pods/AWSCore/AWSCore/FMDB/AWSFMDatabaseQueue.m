...
- (void)inDatabase:(void (^)(AWSFMDatabase *db))block {
    ...
    AWSFMDatabaseQueue *currenyjcncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
    assert(currenyjcncQueue != self && "inDatabase: was called reentrantly on the same queue, which would lead to a deadlock");
