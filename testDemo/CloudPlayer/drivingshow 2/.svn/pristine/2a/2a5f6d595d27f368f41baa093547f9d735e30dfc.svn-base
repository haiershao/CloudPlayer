...
- (void)setUpRecorderView:(DevicePosition)devicePosition{
    ...
    [RecordInterface instance].sessionPreset = AVCaptureSessionPreset1280x720;
    ...
    [[RecordInterface instance]startPreview:nil devicePosition:devicePosition completeHandler:^(AVCaptureSession* avcapturess){
        NSLog(@"wwww---%@",[NSThread currentThread]);
        ...
        [_previewLayer setAffineTransform:CGAffineTransformMakeScale(1.7,1)];
...
-(void)writeToReportServer{
    ...
    NSString *occuraddr = [NSString stringWithFormat:@"%@%@",self.cityModel.SubLocality,self.cityModel.Street];
...
//    NSString *urlStr = [NSString stringWithFormat:@"http://112.124.22.101:38080/illegalreport/restapi/cidonekeyreport?cid=%@&longitude=%f&latitude=%f&occurtime=%@&occuraddr=%@&video_id=%@&store_loc=%@&store_param=%@",identifierForVendor,longitude,latitude,occurtime,occuraddr,video_id,store_loc,store_param];
...

{
    ...
    [self getAddressByLatitude:self.userLocation.coordinate.latitude longitude:self.userLocation.coordinate.longitude];
...
//    HWLog(@"%f==%f",self.userLocation.location.coordinate.latitude,self.userLocation.location.coordinate.latitude);
    [self.mkMapView setCenterCoordinate:userLocation.location.coordinate animated:YES];
    ...
    [self.coder reverseGeocodeLocation:userLocation.location completionHandler:^(NSArray *placemarks, NSError *error) {
        // 第一次发送请求给服务器
        [self mapView:self.mkMapView regionDidChangeAnimated:YES];
...
- (void)getAddressByLatitude:(CLLocationDegrees)latitude longitude:(CLLocationDegrees)longitude{
    ...
    [self.coder reverseGeocodeLocation:location completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        if (error) {
            HWLog(@"--------getAddressByLatitude error%@",error);
        }else{
        
            CLPlacemark *placemark=[placemarks lastObject];
            
            HWLog(@"-----------------%@",placemark.addressDictionary);
            CityInfo *cityModel = [CityInfo mj_objectWithKeyValues:placemark.addressDictionary];
            self.cityModel = cityModel;
            
            HWLog(@"-----%@----%@-----%@",cityModel.Thoroughfare,self.cityModel.SubLocality,cityModel.Street);
            //        self.country.text = cityModel.country;
            //        self.province.text = cityModel.city;
            //        self.subLocatity.text = cityModel.subLocality;
            //        self.street.text = cityModel.street;
        }
...
{
    // 返回nil,意味着交给系统处理
    if (![annotation isKindOfClass:[MapAnnotation class]]) return nil;
    ...
    // 设置图片
    annoView.image = [UIImage imageNamed:@"icon_district_highlighted"];
    return annoView;
}
